...
import { cleanName, clip, MAX_NAME_CHARS, trailOff } from "./names";
...
describe("skin names", () => {
  ...
  it("cuts a long name short inside a sentence, after a whole word when it can", () => {
    ...
    expect(clip("Northwind Traders rebrand, final deliverables")).toBe("Northwind Traders rebrand…");
    expect(clip("Northwind Traders, rebrand", 20)).toBe("Northwind Traders…");
    expect(Array.from(clip("🦊".repeat(40)))).toHaveLength(32);
  });

  it("trails off once, even after a name cut short", () => {
    expect(trailOff(`Writing into ${clip("Taxes")}`)).toBe("Writing into Taxes…");
    expect(trailOff(`Writing into ${clip("Northwind Traders rebrand, final deliverables")}`)).toBe("Writing into Northwind Traders rebrand…");
  });
});
