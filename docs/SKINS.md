...
| art with a transparent background | PNG (`--lossless`) | JPEG has no alpha; the template supplies the folder shape, but transparency inside the art is kept |

Try both when a skin is near the 400 KB limit and keep the smaller file. The built-in set
does exactly this: the photographic recipes ship as JPEG, `paper`, `halftone` and `stripes`
ship as PNG.
