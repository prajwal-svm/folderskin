...
`src/text.ts`). Names can repeat as often as people like, but ids never do. A new id is drawn again
...
| | |
|---|---|
| A pack's pictures | PNG or lossless WebP, 256 to 1024 px a side and 1.5 MB each, with at most 50 pictures and 64 MB in all |
...
`.dev.vars`, where `IP_SALT=anything` is enough for counting), then:
