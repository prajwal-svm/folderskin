...
the `penalties` and `marks` tables, each submission's `network` and the index the pending count
reads; until they are applied, counting, sharing and decisions fail. The `PACKS` binding needs the
