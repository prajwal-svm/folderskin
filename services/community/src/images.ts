...
export type Picture = { format: Format; width: number; height: number };
...
/**
 * The format and size of a PNG, JPEG or WebP picture. Throws, in a sentence, for anything else,
 * for a header that doesn't hold together, and for an animated PNG or WebP: a folder icon is one
 * still picture.
 */
...
function png(b: Uint8Array): Picture {
  ...
  const picture: Picture = { format: "png", width: u32be(b, 16), height: u32be(b, 20) };
...
function jpeg(b: Uint8Array): Picture {
  ...
  while (at + 4 <= b.length) {
    ...
    if (marker >= 0xc0 && marker <= 0xcf && marker !== 0xc4 && marker !== 0xc8 && marker !== 0xcc) {
      ...
      return { format: "jpeg", height: u16be(b, at + 5), width: u16be(b, at + 7) };
    }
...
function webp(b: Uint8Array): Picture {
  ...
  if (first === "VP8 ") {
    ...
    picture = { format: "webp", width: u16le(b, 26) & 0x3fff, height: u16le(b, 28) & 0x3fff };
  } else if (first === "VP8L") {
    // Lossless: a signature byte, then width - 1 and height - 1 in 14 bits each.
    if (b[20] !== 0x2f) throw bad("is a damaged WebP");
    const bits = u32le(b, 21);
    picture = { format: "webp", width: (bits & 0x3fff) + 1, height: ((bits >>> 14) & 0x3fff) + 1 };
  } else if (first === "VP8X") {
    ...
    picture = { format: "webp", width: u24le(b, 24) + 1, height: u24le(b, 27) + 1 };
  } else {
    throw bad("is a damaged WebP");
  }
  // Frames without the flag would still be an animation; no still picture has them.
  let at = 12;
  for (let chunks = 0; at + 8 <= b.length && chunks < 10_000; chunks++) {
