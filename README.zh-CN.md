...
除了少数必须联网的功能，其余一切都留在你的电脑上。没有账号，没有付费墙，也没有任何追踪：FolderSkin 唯一会上报的，是某个社区皮肤包被添加了，而且只附带这个皮肤包的 ID，好让 folderskin.app 显示每个皮肤包被添加了多少次。macOS 版安装后只占约 8.9 MB。
...
想让里面的文件夹也换上同一款皮肤，就打开文件夹名称下方的**包括子文件夹**。FolderSkin 会先数一数一共有多少个（包括所有层级，但不算隐藏文件夹和应用程序包），按钮随之变成**应用到 25 个文件夹**，数字以实际数量为准。超过十个时，开始前会先问你一声。文件夹面板会逐个显示处理好的文件夹，按**停止**会在处理完手头这个之后结束。最后的汇总会告诉你改了哪些、哪些没能改以及原因，并让你选择继续，或者重试那些没改成的文件夹。**全部还原**只会撤掉这次换上的图标，不多也不少。
...
| <img src="docs/images/providers/google.svg" width="20" height="20" alt=""> | [Google Gemini](https://aistudio.google.com/apikey) | Gemini 2.5 Flash Image | 约 $0.04 |
